...
class ProcessAction {
	...
	public function set_from_params( $params ) {
		...
		if ( ! empty( $params['event'] ) ) {
			$this->event = new ProcessEvent( [ 'type' => $params['event']['type'] ] );
		}
	}
	...
	public static function generate_form( ProcessAction $action, string $process_id = '' ): string {
		...
		$html                = '<div class="process-action-form pa-type-' . $action->type . ' pa-status-' . $action->status . '" data-id="' . $action->id . '">';
			...
			$html           .= \OsFormHelper::select_field(
				'process[actions][' . $action->id . '][type]',
				__( 'Action Type', 'latepoint' ),
				\LatePoint\Misc\ProcessAction::get_action_types_for_select(),
				$action->type,
				[
					'class'           => 'process-action-type',
					'data-action-id'  => $action->id,
					'data-process-id' => $process_id,
					'data-route'      => \OsRouterHelper::build_route_name( 'processes', 'load_action_settings' ),
				],
				[ 'class' => 'os-col-10' ] 
			);
			$html           .= \OsFormHelper::select_field(
				'process[actions][' . $action->id . '][status]',
				__( 'Status', 'latepoint' ),
				[
					LATEPOINT_STATUS_ACTIVE   => __( 'Active', 'latepoint' ),
					LATEPOINT_STATUS_DISABLED => __( 'Disabled', 'latepoint' ),
				],
				$action->status,
				false,
				[ 'class' => 'os-col-2' ] 
	...
	public static function generate_settings_fields( ProcessAction $action, string $process_id = '' ) {
		...
		if ( in_array( $action->type, [ 'send_email', 'send_sms' ] ) ) {
			...
			$html .= '<a href="#" data-os-after-call="latepoint_init_template_library" data-os-params="' . \OsUtilHelper::build_os_params(
				[
					'action_id'   => $action->id,
					'action_type' => $action->type,
					'process_id'  => $process_id,
				] 
			) . '" data-os-lightbox-classes="width-1000" data-os-action="' . \OsRouterHelper::build_route_name( 'notifications', 'templates_index' ) . '" href="#" data-os-output-target="side-panel" class="latepoint-btn latepoint-btn-outline latepoint-btn-sm"><i class="latepoint-icon latepoint-icon-book"></i><span>' . __( 'Load from template', 'latepoint' ) . '</span></a>';
		...
		switch ( $action->type ) {
			case 'send_email':
				...
				$html .= \OsFormHelper::text_field(
					'process[actions][' . $action->id . '][settings][to_email]',
					__( 'To Email', 'latepoint' ),
					$action->settings['to_email'],
					[
						'theme'       => 'simple',
						'placeholder' => __( 'To email address', 'latepoint' ),
					],
					[ 'class' => 'os-col-6' ] 
				);
				$html .= \OsFormHelper::text_field(
					'process[actions][' . $action->id . '][settings][subject]',
					__( 'Email Subject', 'latepoint' ),
					$action->settings['subject'],
					[
						'theme'       => 'simple',
						'placeholder' => __( 'Email Subject', 'latepoint' ),
					],
					[ 'class' => 'os-col-6' ] 
				);
				$html .= '</div>';
				$html .= \OsFormHelper::textarea_field(
					'process[actions][' . $action->id . '][settings][content]',
					false,
			...
			case 'send_sms':
				if ( \OsSmsHelper::get_sms_processors() ) {
					$html .= \OsFormHelper::text_field(
						'process[actions][' . $action->id . '][settings][to_phone]',
						__( 'To Phone Number', 'latepoint' ),
						$action->settings['to_phone'],
						[
							'theme'       => 'simple',
					...
					$html .= \OsFormHelper::textarea_field(
						'process[actions][' . $action->id . '][settings][content]',
						__( 'Message Content', 'latepoint' ),
						$action->settings['content'],
						[
							'theme'       => 'simple',
							'placeholder' => __( 'Message', 'latepoint' ),
							'rows'        => 4,
						] 
					);
				} else {
	...
	public function settings_form() {
		...
		switch ( $this->type ) {
			case 'send_email':
				$html .= \OsFormHelper::text_field(
					'action[actions][' . $this->id . '][to]',
					'',
					$this->settings['to'],
					[
						'theme'       => 'simple',
						'placeholder' => __( 'Email To', 'latepoint' ),
					] 
				);
				break;
			case 'send_sms':
				$html .= \OsFormHelper::text_field(
					...
					[
						'theme'       => 'simple',
						'placeholder' => __( 'SMS To', 'latepoint' ),
					] 
				);
				break;
			case 'send_whatsapp':
				$html .= \OsFormHelper::text_field(
					...
					[
						'theme'       => 'simple',
						'placeholder' => __( 'WhatsApp Message To', 'latepoint' ),
					] 
				);
				break;
			case 'trigger_webhook':
				$html .= \OsFormHelper::text_field(
					'action[actions][' . $this->id . '][url]',
					'',
					$this->settings['url'],
					[
						'theme'       => 'simple',
						'placeholder' => __( 'Webhook URL', 'latepoint' ),
					] 
				);
				break;
		}
					...
					if ( $content_type == 'buttons' ) {
						if ( ! empty( $content ) ) {
							foreach ( $content as $button ) {
								...
								if ( $button['type'] == 'URL' ) {
									$button['url'] = empty( $this->settings['variables'][ $content_type ] ) ? $button['url'] : \OsReplacerHelper::replace_all_vars( str_replace( array_keys( $this->settings['variables'][ $content_type ] ), array_values( $this->settings['variables'][ $content_type ] ), $button['url'] ), $this->replacement_vars );
								}
	...
	public function generate_preview() {
		if ( empty( $this->prepared_data_for_run ) ) {
			...
			if ( empty( $this->prepared_data_for_run ) ) {
				return '<div class="action-preview-error">' . __( 'You have to create a booking to be able to test this action.', 'latepoint' ) . '</div>';
			}
		}
