...
class User {
	...
	protected array $allowed_records = [
		'agent'    => [],
		'service'  => [],
		'location' => [],
	];
	...
	public function check_if_allowed_record_id( $id, string $model_type ) {
		...
		if ( $this->are_all_records_allowed( $model_type ) ) {
			return $id;
		}
		if ( array_intersect( [ $id ], $this->get_allowed_records( $model_type ) ) ) {
			return $id;
		}
		return false;
	}
			...
			case LATEPOINT_USER_TYPE_CUSTOM:
				...
				if ( empty( $custom ) ) {
					...
					foreach ( $allowed_connections as $allowed_connection ) {
						if ( ! in_array( $allowed_connection->agent_id, $this->allowed_records['agent'] ) ) {
							$this->allowed_records['agent'][] = $allowed_connection->agent_id;
						}
						if ( ! in_array( $allowed_connection->service_id, $this->allowed_records['service'] ) ) {
							$this->allowed_records['service'][] = $allowed_connection->service_id;
						}
						if ( ! in_array( $allowed_connection->location_id, $this->allowed_records['location'] ) ) {
							$this->allowed_records['location'][] = $allowed_connection->location_id;
						}
					}
				}
				break;
	...
	public function get_avatar_url(): string {
		...
		if ( $this->backend_user_type ) {
			if ( $this->agent ) {
				return $this->agent->get_avatar_url();
			}
			if ( $this->wp_user ) {
				return get_avatar_url( $this->wp_user->user_email );
			}
		}
		if ( $this->customer ) {
