...
class OsRolesHelper {
	...
	public static function get_capabilities_for_all_controllers() {
		if ( isset( self::$capabilities_for_controllers ) ) {
	...
	public static function can_user_make_action_on_model_record( OsModel $model, string $action ) {
		if ( OsAuthHelper::get_current_user()->backend_user_type == LATEPOINT_USER_TYPE_ADMIN ) {
			return true; // admins are allowed to do everything
		}
		...
		if ( self::can_user_perform_model_action( get_class( $model ), $action ) ) {
			switch ( get_class( $model ) ) {
				case 'OsBookingModel':
					if ( OsAuthHelper::get_current_user()->are_all_records_allowed( 'agent' ) ) {
						return true;
					}
					$allowed_ids = OsAuthHelper::get_current_user()->get_allowed_records( 'agent' );
					if ( $allowed_ids && in_array( $model->agent_id, $allowed_ids ) ) {
						return true;
					}
					break;
				case 'OsCustomerModel':
					...
					if ( $allowed_ids ) {
						...
						$has_bookings_with_allowed_agents = $bookings->select( 'id' )->where(
							[
								'agent_id'    => $allowed_ids,
	...
	public static function get_capabilities_list_for_agent_role() {
		...
		if ( empty( $capabilities ) ) {
			$capabilities = self::get_default_capabilities_list_for_agent_role();
		}
		/**
	...
	public static function save_from_params( array $role_params ) {
		...
		if ( empty( $role->wp_role ) ) {
			return new WP_Error( 'invalid_wp_role', __( 'WP Role invalid', 'latepoint' ) );
		}
	...
	public static function delete( string $wp_role ): bool {
		...
		if ( in_array( $wp_role, [ LATEPOINT_WP_ADMIN_ROLE, LATEPOINT_WP_AGENT_ROLE ] ) ) {
			return false;
		}
	...
	public static function get_all_available_actions_list() {
		$actions = [
			...
			'location__edit' ,
			'service_extra__view',
			'service_extra__delete' ,
			'service_extra__create',
			'service_extra__edit' ,
			'booking__view',
			'booking__delete' ,
			'booking__create',
	...
	public static function get_users_for_role( \LatePoint\Misc\Role $role ): array {
		...
		switch ( $role->user_type ) {
			...
			case LATEPOINT_USER_TYPE_CUSTOM:
				if ( $role->wp_role ) {
					$wp_users = get_users( [ 'role' => $role->wp_role ] );
				}
				break;
		}
