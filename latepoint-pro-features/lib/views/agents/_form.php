				...
				if ( $agent->is_new_record() ) {
					...
					if ( OsRolesHelper::can_user( 'agent__delete' ) || OsRolesHelper::can_user( 'agent__edit' ) ) {
						...
						if ( OsRolesHelper::can_user( 'agent__delete' ) ) {
							$extra_actions_html .= '<div class="os-context-item os-danger os-delete-confirm" data-os-confirm-title="' . esc_attr__( 'Delete Agent', 'latepoint-pro-features' ) . '"
                            ...
                            data-os-action="' . esc_attr( OsRouterHelper::build_route_name( 'agents', 'destroy' ) ) . '"><i class="latepoint-icon latepoint-icon-trash-2"></i><span>' . esc_html__( 'Delete', 'latepoint-pro-features' ) . '</span></div>';
						}
