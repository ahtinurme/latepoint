...
use LatePoint\GoogleCalendarAddon\GuzzleHttp\Promise as P;
use LatePoint\GoogleCalendarAddon\GuzzleHttp\Promise\FulfilledPromise;
use LatePoint\GoogleCalendarAddon\GuzzleHttp\Promise\PromiseInterface;
use LatePoint\GoogleCalendarAddon\GuzzleHttp\Psr7\LazyOpenStream;
use LatePoint\GoogleCalendarAddon\GuzzleHttp\TransferStats;
use LatePoint\GoogleCalendarAddon\GuzzleHttp\Utils;
use LatePoint\GoogleCalendarAddon\Psr\Http\Message\RequestInterface;
use LatePoint\GoogleCalendarAddon\Psr\Http\Message\UriInterface;
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private static function createRejection(EasyHandle $easy, array $ctx): PromiseInterface
    {
        ...
        if ('' !== $sanitizedError) {
            $redactedUriString = \LatePoint\GoogleCalendarAddon\GuzzleHttp\Psr7\Utils::redactUserInfo($uri)->__toString();
    ...
    private static function sanitizeCurlError(string $error, UriInterface $uri): string
    {
        ...
        $redactedUriString = \LatePoint\GoogleCalendarAddon\GuzzleHttp\Psr7\Utils::redactUserInfo($baseUri)->__toString();
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf): void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \LatePoint\GoogleCalendarAddon\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \LatePoint\GoogleCalendarAddon\GuzzleHttp\Psr7\Utils::streamFor($sink);
